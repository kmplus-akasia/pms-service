CREATE TABLE IF NOT EXISTS kpi_log_v3 (
  kpi_log_id BIGINT AUTO_INCREMENT PRIMARY KEY,
  ...
  source ENUM('SYSTEM','MIGRATION', 'MANUAL', 'DICTIONARY', 'PKPI') DEFAULT 'SYSTEM',
  ...
  CONSTRAINT fk_kpi_log
    FOREIGN KEY (kpi_id) REFERENCES kpi_v3(kpi_id),

  CONSTRAINT fk_kpi_log_parent
    FOREIGN KEY (parent_kpi_id)
    REFERENCES kpi_v3(kpi_id),

  CONSTRAINT fk_kpi_log_item_approver
    FOREIGN KEY (item_approver_employee_number)
    REFERENCES tb_employee(employee_number),

  CONSTRAINT fk_kpi_log_created_by
    FOREIGN KEY (created_by_employee_number)
    REFERENCES tb_employee(employee_number),

  INDEX idx_kpi_log_parent (parent_kpi_id),
  INDEX idx_kpi_log_created_by (created_by_employee_number),
  INDEX idx_kpi_log_item_approver (item_approver_employee_number)
